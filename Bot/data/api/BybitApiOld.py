...
class BybitApiOld(BrokerApi):
    ...
    def have_order_long(self, currency_name) -> bool:
        return self.__have_order(currency_name, BybitApiOld.PositionType.LONG)

    def have_order_short(self, currency_name) -> bool:
        return self.__have_order(currency_name, BybitApiOld.PositionType.SHORT)
    ...
    def close_short_position(self, currency_name):
        side = "Buy"
        self.__close_position(currency_name, side, BybitApiOld.PositionType.SHORT.value)

    # api.close_position(name_coin_u, "Sell", 0)
    def close_long_position(self, currency_name):
        side = "Sell"
        self.__close_position(currency_name, side, BybitApiOld.PositionType.LONG.value)
